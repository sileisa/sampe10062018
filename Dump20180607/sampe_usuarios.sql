...
CREATE TABLE `usuarios` (
  ...
  `SobrenomeUsuario` longtext NOT NULL,
  `Login` longtext NOT NULL,
  `Senha` longtext NOT NULL,
  `Hierarquia` longtext,
  `CargoId` int(11) NOT NULL,
  PRIMARY KEY (`UsuarioId`),
  KEY `IX_CargoId` (`CargoId`) USING HASH,
  CONSTRAINT `FK_Usuarios_Cargoes_CargoId` FOREIGN KEY (`CargoId`) REFERENCES `cargoes` (`CargoId`) ON DELETE CASCADE ON UPDATE CASCADE
